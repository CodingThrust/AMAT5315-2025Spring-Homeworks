mutable struct SimulatedBifurcation{T, KIND}
    ...
    const h::Vector{T}
    const J::Vector{T}
    function SimulatedBifurcation{KIND}(a::T, c0::T, g::SimpleGraph{Int}, h::Vector{T},  J::Vector{T}) where {T, KIND}
        ...
        @assert length(h) == nv(g) "Length of h must be equal to the number of vertices in the graph, got $(length(h)) and $(nv(g))"
        return new{T, KIND}(a, c0, g, h, J)
    end
end
function SimulatedBifurcation{KIND}(g::SimpleGraph{Int}, h::Vector{T}, J::Vector{T}; c0=T(0.5/sqrt(nv(g))/norm(J))) where {T, KIND}
    @assert KIND in (:aSB, :bSB, :dSB) "Invalid bifurcation type: $KIND, must be one of (:aSB, :bSB, :dSB)"
    @assert length(J) == ne(g) "Length of J must be equal to the number of edges in the graph, got $(length(J)) and $(ne(g))"
    @assert length(h) == nv(g) "Length of h must be equal to the number of vertices in the graph, got $(length(h)) and $(nv(g))"
    return SimulatedBifurcation{KIND}(T(1.0), c0, g, h, J)
end

function potential_energy(sys::SimulatedBifurcation{T, :aSB}, x::Vector{T}) where T
    return sum(xi -> xi^4 / 4 + sys.a/2 * xi^2, x) .- sys.c0 * sum(coupling * x[src(e)] * x[dst(e)] for (e, coupling) in zip(edges(sys.g), sys.J) ) - sum(sys.h .* x)
end
