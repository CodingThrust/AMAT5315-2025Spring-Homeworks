...
plot(
    T,
    energy_gap[:, 1],
    label = "triangles,sites=$(N0[1])",
    xlabel = "Temperature",
    ylabel = "Energy gap",
    title = "Energy gap vs Temperature",
    yscale = :log10,
)
...
function generate_p2(Nvec::Vector{Vector{Int64}}, T::S) where {S}
    ...
    for n ∈ 1:3
        for i ∈ 1:length(N[n])
            ...
            N = Nvec[n][i]
            A = generate_topology_matrix(N, model[n])
            H = generate_trans(A, T)
            E, _ = eigsolve(H, normalize(rand(2^N)), eignum, :LM, KrylovKit.Arnoldi(;tol=1e-16))
...
plot(
    ...
    ylabel = "Energy gap",
    title = "Energy gap vs Number of sites with T=0.1",
)
