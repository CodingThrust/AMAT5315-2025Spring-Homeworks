...
## Q2
Tropical(1.0) |> typeof
Tropical(1.0) |> typeof |> supertypes

## Q3
Tropical |> isconcretetype

## Q4
Tropical{Real} |> isconcretetype
...
# Task 2
struct SetSemiring{T <: Integer} <: AbstractSemiring
    elements::Set{T}

    function SetSemiring(s::Set{T}) where T
        new{T}(Set{T}(s))
    end
    #function SetSemiring(elements...)
    #    new{T}(Set{T}([elements...]))
    #end
    function SetSemiring(elements::Vector{T}) where T
        new{T}(Set{T}(elements))
    end
end

Base.promote_type(::Type{SetSemiring{T1}}, b::Type{SetSemiring{T2}}) where {T1, T2} = SetSemiring{promote_type(T1,T2)}

Base.show(io::IO, s::SetSemiring) = Base.print(io, "$(s.elements)ₛ")

Base.:+(a::SetSemiring, b::SetSemiring) = SetSemiring(union(a.elements, b.elements))
# multiplication is mapped to element-wise Integer multiplication
Base.:*(a::SetSemiring, b::SetSemiring) = SetSemiring([σ * τ for σ in a.elements for τ in b.elements])

Base.zero(::Type{SetSemiring{T}}) where T = SetSemiring(Set{T}())
Base.zero(::SetSemiring{T}) where T = zero(SetSemiring{T})

Base.one(::Type{SetSemiring{T}}) where T = SetSemiring(Set{T}([1]))
Base.one(::SetSemiring{T}) where T = one(SetSemiring{T})

Base.:(==)(a::SetSemiring, b::SetSemiring) = a.elements == b.elements

## test cases
using Test
@testset "SetSemiring" begin
    @test SetSemiring([2]) + SetSemiring([5, 4]) == SetSemiring([5, 4]) + SetSemiring([2]) == SetSemiring([2, 4, 5])
    ...
    @test SetSemiring([2]) * one(SetSemiring{Int}) == SetSemiring([2])
end
